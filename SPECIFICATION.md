...
§11 and §7. As of 2026-09-07 the unit suite is 844 passing, 0 skipped, and the GPU
...
| 2026-09-06 | Decimation names the target it missed (correct, per the 2026-09-05 row) but the reason it gives is false: reducing the clean Menger sponge stopped at 736 triangles and introduced 32 self-intersections while the summary said further collapses "would have created invalid geometry". A quality-collapse loop that refuses individual collapses on a *local* validity test still has to be checked against the whole-mesh invariants afterwards — the app's own status bar reported the 32 issues in the same frame the panel denied them. **Fixed 2026-09-07**, see the rows for that date. (This row originally read "734 triangles and 67 self-intersections"; measured on 2026-09-07, in the running app and in the shipping code path, it is 736 and 32, and no reduction of this mesh lands on 734 triangles at all) |
...
| 2026-09-07 | **An edge collapse is now checked against the whole mesh, not just its one-ring** (item 22). `Reducer`'s only validity test is `collapse_creates_flip_or_invalid`, which walks the edge's one-ring for a normal flip and a link-condition violation, and *none* of the four defects decimation was adding is a one-ring property: the two triangles that end up crossing are on opposite walls and share no vertex; the degenerate threshold is scale-relative to the whole mesh and moves as decimation proceeds; a kept vertex can land on a vertex that is nowhere near it in the connectivity graph; and the non-manifold edges follow from that duplicate, because `PositionTopology` measures topology by position. The reducer had refused exactly the collapses it could see and made the rest — the message was not a lie about what it refused, it was a claim about collapses it never examined. `ValidatingReducer` supplies the missing half through a new `Reducer.CollapseIsGloballyValid` hook (a documented vendor deviation, default `true` = upstream). On the sponge, 15 of 21 targets came back with defects before and none does now |
| 2026-09-07 | Running `ResolveSelfIntersectionsOperation` after every decimation was **considered and rejected**. It is the cheap fix and the operation already works, but it would silently change the geometry the user asked to *simplify*, it treats the symptom while leaving decimation free to keep making collapses it has no business making, and it addresses one of the four defect categories. Rejecting the collapse is the fix; resolving afterwards would have been a way of not looking |
| 2026-09-07 | The guard refuses only **new** defects. A collapse in a region that already self-intersects, or one reshaping an already-degenerate triangle, still goes ahead: preserving the validity a mesh had is the promise, not demanding validity it never had, which would leave a broken scan — the common case for this app's users — barely decimated. The visible consequence is that on an already-broken mesh the guarded pass can leave a *higher* defect count than the unguarded one (`lg-camel_b.obj`: 1,450 against 692), because the unguarded pass destroys some bad geometry along with everything else. That is the right trade and it is not a repair; Auto Repair is |
| 2026-09-07 | Every threshold the guard tests is **read from the detector that would report the defect** — `DegenerateTriangleDetector.AreaEpsilonFor`, `DuplicateVertexDetector.IsCoincident`, `SelfIntersectionSearch`'s degenerate exclusion, all made public for this. Same reasoning as the hole-fill fix the same day: a guard with its own copy of a rule is a second rule, and two rules is the bug |
| 2026-09-07 | The item-22 tests pin **both halves on purpose**. Refusing more collapses makes every "no more defects than before" assertion pass, and a decimator that decimates nothing is perfectly valid and perfectly useless, so `ATargetTheMeshCanReach_IsStillReachedExactly` and `APercentageTargetOnAMeshWithRoomToLose_IsStillMet` are as load-bearing as the validity assertions. `TheFixtureStillReproducesTheBug_WithoutTheGuard` asserts the *bare* reducer still breaks the fixture, so the validity tests cannot go vacuous — the same guard the corpus seam test needed |
| 2026-09-07 | Correctness cost time and the number is recorded rather than hidden: the 139,989-triangle Eiffel tower sample goes from 1.3 s to 23 s for a 50% reduction, the 2,112-triangle sponge from 90 ms to 1.9 s. Four rounds of optimisation took the tower down from 217 s — one broadphase query per collapse instead of per reshaped triangle, a dirty-set scheme in place of displacement-padded stale queries, a cell budget on the grid over that set, and reusing the intersection-test object instead of allocating millions. Decimation runs off the UI thread, so §6.4's 100 ms UI budget is unaffected and §6.4 sets no target for simplification itself; making it faster (the obvious lever is parallelising the candidate tests) is its own slice |
...
22. ~~**Decimation introduces the invalid geometry it claims to have avoided**~~
    — done 2026-09-07, see
    `reports/M4/20260907T210000Z-decimate-validity/report.md`. The measured case
    was **736 triangles and 32 self-intersections** from the clean Menger sponge
    (this entry previously said 734 and 67; both figures were wrong and are
    corrected here, in the §11 row for 2026-09-06 and in `docs/usage.html`), and
    intermediate targets were worse — 1,400 triangles came back with 58 defects
    across four categories and an unqualified success message.
    `ValidatingReducer` now rejects, through a new `Reducer.CollapseIsGloballyValid`
    hook, any collapse that would add a defect the mesh did not already have,
    tested against the whole mesh and using the detectors' own thresholds. 15 of
    21 targets on the sponge came back broken before and none does now; every
    target the mesh can honestly reach is still reached exactly, and the honest
    floor moved only from 736 triangles to 776. See §11.
