...
- Split to fit the printer: a plane cut can generate registration pins, so the
  halves locate themselves when you glue them back together
- Judge it before you slice it: a build plate sized to your printer that warns
  which side the model overhangs, orthographic and perspective projection, seven
  view presets, wireframe and x-ray

Status: **in active development** — M0 (Skeleton), M1 (Inspect), M2 (Repair)
and M3 (Edit) are complete; M4 (Polish and release) is underway, with the
viewport/UX block most of the way through (camera and display modes and the
build plate are in; the cross-section slider is not). No public release yet.
See [SPECIFICATION.md](SPECIFICATION.md) for the full roadmap,
